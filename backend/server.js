...
const validateProductionEnv = () => {
  ...
  const required = ['MONGODB_URI', 'JWT_SECRET', 'CORS_ORIGIN', 'FRONTEND_URL', 'SMTP_USER', 'SMTP_PASS', 'OPENAI_API_KEY'];
  const missing = required.filter((key) => !process.env[key]);
  if (missing.length) throw new Error(`Missing required production environment variables: ${missing.join(', ')}`);
  if (process.env.JWT_SECRET.length < 32 || process.env.JWT_SECRET.includes('change-this')) {
    throw new Error('JWT_SECRET must be a unique random secret of at least 32 characters in production');
  }
};
...
app.use((req, res, next) => {
  ...
  const hasAuthCookie = /(?:^|;\s*)auth_token=/.test(req.headers.cookie || '');
  const hasBearerAuth =
  ...
  if (referer) {
    try {
      ...
      if (corsOrigins.includes(refererOrigin)) return next();
    } catch {
...
const globalMax = parseInt(process.env.RATE_LIMIT_MAX, 10) || 200;
...
const loginLimiter = rateLimit({
  ...
  skip: (req, res) => {
    ...
    return req.path === '/me' || req.path === '/auth/me';
  },
  standardHeaders: true,
  legacyHeaders: false,
});
...
const protectedRouteLimiter = rateLimit({
  ...
  max: 100, // 100 requests per minute (much more lenient)
  message: {
    success: false,
...
const applyAuthRateLimiting = (req, res, next) => {
  ...
  if (req.path === '/login' || req.path === '/register' || req.path === '/forgot-password' || req.path === '/reset-password') {
...
app.use('/cars', carRoutes); // Frontend compatibility
app.use('/api/auth', applyAuthRateLimiting, authRoutes);
app.use('/auth', applyAuthRateLimiting, authRoutes);
...
const start = async () => {
  try {
    ...
    const server = app.listen(PORT, () => {
      console.log('');
      console.log('  ==================================================');
      console.log('       Elite Cars Showroom Server');
      console.log('  ==================================================');
